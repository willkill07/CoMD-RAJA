...
int eamForce(SimFlat *s) {
  ...
  if (pot->forceExchange == NULL) {
    ...
    pot->forceExchangeData = comdMalloc<ForceExchangeData>(1);
                ...
                RAJA::simd_exec>>>(
                  ...
                  [=](int iOff, int jOff) {
                    ...
                    if (r2 <= rCut2 && r2 > 0.0) {
                      ...
                      pot->phi->interpolate(r, &phiTmp, &dPhi);
                      pot->rho->interpolate(r, &rhoTmp, &dRho);
...
/*{
  ...
  RAJA::forall<OuterExecPol>(0, s->boxes->nLocalBoxes, [=](int iBox) {
    RAJA::forall<InnerExecPol>(0, s->boxes->nAtoms[iBox], [=](int i) {
      ...
      pot->f->interpolate(pot->rhobar[iOff], &fEmbed, &dfEmbed);
...
void eamDestroy(BasePotential **pPot) {
  ...
  if (!pot)
    return;
  pot->phi->~InterpolationObject();
  pot->rho->~InterpolationObject();
  pot->f->~InterpolationObject();
...
/*
  ...
  table->values = comdCalloc<real_t>(1, n + 3);
  assert(table->values);
...
void bcastInterpolationObject(InterpolationObject **table) {
  ...
  bcastParallel(&buf, sizeof(Buffer), 0);
...
void eamReadSetfl(EamPotential *pot, const char *dir, const char *potName) {
  ...
  new (pot->f) InterpolationObject(nRho, x0, dRho, buf);

  // Read electron density rho(r)
  for (int ii = 0; ii < nR; ++ii)
    fscanf(potFile, FMT1, buf + ii);
  new (pot->f) InterpolationObject(nR, x0, dR, buf);
  ...
  new (pot->phi) InterpolationObject(nR, x0, dR, buf);
...
void eamReadFuncfl(EamPotential *pot, const char *dir, const char *potName) {
  ...
  for (int ii = 0; ii < nR; ++ii)
    fscanf(potFile, FMT1, buf + ii);
  new (pot->rho) InterpolationObject(nR, x0, dR, buf);
