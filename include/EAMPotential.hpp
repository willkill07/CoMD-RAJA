...
struct InterpolationObject {
  ...
  InterpolationObject &
  operator=(InterpolationObject const &) = default;
  ...
  void
  interpolate(real_t r, real_t *f, real_t *df);
...
struct EAMPotential : public BasePotential<EAMPotential> {
  ...
  void
  prepare(SimFlat<EAMPotential> &s);
  int
  force_impl(SimFlat<EAMPotential> &s);
  void
  print_impl(FILE *file);

  friend EAMPotential
  make_eam_setfl(const char *, const char *);
  friend EAMPotential
  make_eam_funcfl(const char *, const char *);
